...
@st.cache_data(ttl=3600)  # 1h — daily curve, updated each business day
def boe_gilt_yields():
    """UK nominal spot (zero-coupon) gilt yields from the Bank of England IADB.
    Series IUDMNZC gives a full curve: 0.5Y, 1Y, 1.5Y, 2Y … 10Y … 25Y.
    Returns dict {tenor_label: float} e.g. {'1Y': 4.23, '2Y': 4.41, '10Y': 4.82}
    or {} on failure.  Plain HTTP — works on Streamlit Cloud, no auth needed.
    Source: bankofengland.co.uk (free, non-commercial use permitted)."""
    try:
        from_date = (dt.date.today() - dt.timedelta(days=14)).strftime("%-d/%b/%Y")
        url = (
            "https://www.bankofengland.co.uk/boeapps/database/"
            "_iadb-fromshowcolumns.asp?csv.x=yes"
            "&SeriesCodes=IUDMNZC&UsingCodes=Y&CSVF=TN"
            f"&Datefrom={from_date}&Dateto=01/Jan/2099"
        )
        r = requests.get(url, headers=H, timeout=20)
        if r.status_code != 200 or not r.text.strip():
            return {}
        lines = [ln for ln in r.text.splitlines() if ln.strip()]
        # Row 1 is blank or title; row 2 has the column headers like:
        # DATE,0.5,1,1.5,2,...,10,...,25
        # Find the header row (starts with 'DATE')
        hdr_i = next((i for i, l in enumerate(lines)
                      if l.upper().startswith("DATE")), None)
        if hdr_i is None:
            return {}
        headers = [h.strip() for h in lines[hdr_i].split(",")]
        # Data rows come after; take the last non-empty one
        data_rows = [l for l in lines[hdr_i + 1:] if l.strip() and
                     not l.strip().startswith("Date")]
        if not data_rows:
            return {}
        last = data_rows[-1].split(",")
        result = {}
        tenor_map = {"1": "1Y", "2": "2Y", "10": "10Y"}
        for col, label in tenor_map.items():
            if col in headers:
...
@st.cache_data(ttl=3600)  # 1h — RBI publishes intraday updates
def india_gsec_yields():
    """India Government Securities yields from worldgovernmentbonds.com —
    a server-rendered page (plain HTTP, no JS needed, works on Streamlit Cloud).
    Parses the yield table and returns dict {tenor: float} e.g.
    {'1Y': 5.92, '2Y': 6.25, '10Y': 6.88}.  Falls back to {} on failure."""
    try:
        r = requests.get("https://www.worldgovernmentbonds.com/country/india/",
                         headers=_BROWSER_H, timeout=20)
        if r.status_code != 200:
            return {}
        txt = re.sub(r"\s+", " ", _html.unescape(_TAGS.sub(" ", _STRIP.sub(" ", r.text))))
        # Table rows look like: "1 Year 5.92 % ..." or "2 Years 6.25 % ..."
        result = {}
        tenor_patterns = [
            (r"1\s*Year\b[^%]{0,40}?(\d+\.\d{2})\s*%", "1Y"),
            (r"2\s*Year[s]?\b[^%]{0,40}?(\d+\.\d{2})\s*%", "2Y"),
            (r"10\s*Year[s]?\b[^%]{0,40}?(\d+\.\d{2})\s*%", "10Y"),
        ]
        for pattern, label in tenor_patterns:
...
# ---------------------------------------------------------------------------
# Legacy browser-based scrapers kept for local use; on Streamlit Cloud these
# return None immediately (Playwright not installed).
# ---------------------------------------------------------------------------
def bond_yield_browser(url):
    """Headless-browser scrape — works locally only, not on Streamlit Cloud."""
    try:
        from playwright.sync_api import sync_playwright
    except Exception:
        return None
...
def bond_yield_change_browser(url):
    """Headless-browser scrape with daily change — works locally only."""
    try:
        from playwright.sync_api import sync_playwright
    except Exception:
        return (None, None)
...
/* table card — scrolls sideways on small screens instead of clipping columns */
.dash-wrap{border:1px solid rgba(255,255,255,.08);border-radius:14px;
...
/* slim scrollbar so the horizontal swipe area looks intentional */
.dash-wrap::-webkit-scrollbar{height:7px}
.dash-wrap::-webkit-scrollbar-thumb{background:rgba(255,255,255,.18);border-radius:8px}

/* ---- phones / small screens: same UI, tighter so tables fit & swipe cleanly ---- */
@media (max-width:640px){
  ...
  /* two summary cards per row instead of one cramped column */
...
# UK: Bank of England IADB nominal spot gilt curve (free daily CSV, plain HTTP).
# India: worldgovernmentbonds.com G-Sec table (server-rendered, plain HTTP).
# Both work on Streamlit Cloud — no headless browser or paid API needed.
_boe = boe_gilt_yields()    # {'1Y': float, '2Y': float, '10Y': float}
_in  = india_gsec_yields()  # {'1Y': float, '2Y': float, '10Y': float}

# Keep YIELD_PAGES / YIELD_10Y_PAGES so the expander below still works.
YIELD_PAGES = {
    "uk_1y":    "BoE IADB IUDMNZC (nominal spot gilt curve)",
    "uk_2y":    "BoE IADB IUDMNZC (nominal spot gilt curve)",
    "india_1y": "worldgovernmentbonds.com/country/india/",
    "india_2y": "worldgovernmentbonds.com/country/india/",
    "japan_1y": "Investing.com (browser, local only)",
    "japan_2y": "Investing.com (browser, local only)",
}
YIELD_10Y_PAGES = {
    "india": "worldgovernmentbonds.com/country/india/",
    "uk":    "BoE IADB IUDMNZC (nominal spot gilt curve)",
    "japan": "Investing.com (browser, local only)",
}

# Merge scraped values into _yld / _yld10 dicts (same keys as before)
_yld = {
    "india_1y": _in.get("1Y"),
    "india_2y": _in.get("2Y"),
    "uk_1y":    _boe.get("1Y"),
    "uk_2y":    _boe.get("2Y"),
    "japan_1y": bond_yield_browser(
        "https://www.investing.com/rates-bonds/japan-1-year-bond-yield"),
    "japan_2y": bond_yield_browser(
        "https://www.investing.com/rates-bonds/japan-2-year-bond-yield"),
}
# 10Y: prefer scraped (fresh) → fallback to FRED monthly via auto_val
_yld10 = {
    "india": (_in.get("10Y"), None),
    "uk":    (_boe.get("10Y"), None),
    "japan": bond_yield_change_browser(
...
def ycell(key):
    v = _yld.get(key)
    return f"{v:.2f}%" if v is not None else gap("no free feed")
...
def _us_cell(key, fred_id):
    """US tenor from treasury.gov; if that was blocked/empty (common on cloud),
    fall back to the daily FRED series so US never goes blank."""
    v = ty.get(key, (None, None))[0]
    if v is not None:
        return f"{v:.2f}%"
    fv, _ = fred_latest(fred_id, FRED_API_KEY)   # skips '.' (holiday) observations
    return f"{fv:.2f}%" if fv is not None else gap()


render_table(["Country", "1Y", "2Y", "10Y"], [
    ["India", ycell("india_1y"), ycell("india_2y"), _in10],
    ["US", _us_cell("1Y", "DGS1"), _us_cell("2Y", "DGS2"), _us_cell("10Y", "DGS10")],
    ["UK", ycell("uk_1y"), ycell("uk_2y"), _uk10],
    ["Germany", _de_cell("1Y"), _de_cell("2Y"), _de_cell("10Y")],
    ["Japan", _jp_cell("1Y"), _jp_cell("2Y"), _jp10],
])
st.markdown('<div class="src">US = Treasury.gov (FRED fallback); Germany = ECB '
            'euro-area AAA curve; Japan = MOF JGB curve. '
            'UK 1Y/2Y/10Y = Bank of England IADB nominal spot gilt curve (IUDMNZC series, '
            'free daily CSV). '
            'India 1Y/2Y/10Y = worldgovernmentbonds.com G-Sec table (plain HTTP). '
            'Both work on Streamlit Cloud — no browser needed.</div>',
            unsafe_allow_html=True)
...
with st.expander("Yield scrape details (verify the picked value)"):
    st.caption(
        "UK: Bank of England IADB (IUDMNZC series — nominal spot gilt curve). "
        "India: worldgovernmentbonds.com G-Sec table. "
        "Japan: Investing.com via headless browser (local only; MOF CSV for 10Y). "
        "A gap means the source was unreachable or returned no data today."
    )
...
render_table(["Indicator", "Current", "Change"], [
    ["India VIX", num(_iv[0]) if _iv else gap(), chg(_iv[1]) if _iv else ""],
    ["US VIX (CBOE)", num(_uv[0]) if _uv else gap(), chg(_uv[1]) if _uv else ""],
])
