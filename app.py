...
def chart_png(dates, cols):
    ...
    for c in cols:
        ...
        ax.plot(x, ys, label=c, linewidth=1.5)
...
def build_pdf(report, meta):
    ...
    def para(x, hdr=False):
        # newline (price over change) -> <br/> so cells stack inside the PDF too
        return Paragraph(_pdf_safe(x).replace("\n", "<br/>"),
...
for region, name, tk in EQUITIES:
    ...
    rows.append([region, name, cur] + pcells(d, ("m1", "m3", "m6", "y1", "ytd")))
render_table(["Region", "Index", "Current", "1M", "3M", "6M", "1Y", "YTD"], rows)
...
def ycell(key):
    v = _yld.get(key)
    return f"{v:.2f}%" if v is not None else gap("scrape failed")

def ten_y_cell(country):
    """(10Y value cell, daily-change cell) for a scraped country. Uses the live
    scraped value+change; falls back to the FRED monthly value (change blank)
    if the scrape misses, so the row degrades gracefully instead of going empty."""
...
render_table(["Country", "1Y", "2Y", "10Y", "Daily change (10Y)"], [
    ["India", ycell("india_1y"), ycell("india_2y"), _in10, _inchg],
    ["US", num(ty["1Y"][0], 2) + "%" if ty["1Y"][0] else gap(),
     num(ty["2Y"][0], 2) + "%" if ty["2Y"][0] else gap(),
     num(ty["10Y"][0], 2) + "%" if ty["10Y"][0] else gap(),
     chg(ty["10Y"][1]*100, bps=True, inverse=True) if ty["10Y"][1] is not None else gap()],
    ["Japan", ycell("japan_1y"), ycell("japan_2y"), _jp10, _jpchg],
])
        ...
        try:
            ...
            _labels = (_altchart.Chart(_marks)
                       ...
                       .encode(x=_x, y="yield:Q", color=_color,
                               text=_altchart.Text("yield:Q", format=".1f")))
...
with st.expander("Yield scrape details (verify the picked value)"):
    ...
    for k, u in YIELD_PAGES.items():
        v = _yld.get(k)
        st.markdown(f"**{k}** — {v:.3f}% from {u}" if v is not None
                    else f"**{k}** — no value read from {u}")
    for k, u in YIELD_10Y_PAGES.items():
        v, c = _yld10.get(k, (None, None))
        if v is not None:
            cs = (f", daily change {c:+.3f} ({c*100:+.0f} bps)" if c is not None
                  else ", daily change n/a (no prev-close read)")
            st.markdown(f"**{k}_10y** — {v:.3f}%{cs} from {u}")
        else:
...
gdp = worldbank("NY.GDP.MKTP.KD.ZG")        # real GDP growth %, annual
fxr = worldbank("FI.RES.TOTL.CD")           # total reserves incl gold, US$, annual
wpi = worldbank("FP.WPI.TOTL")              # wholesale price index, annual
render_table(["Indicator", "Latest"], [
    ["India CPI", auto_val("india_cpi")],
    ["India WPI (index)", f"{wpi[0]:,.1f} ({wpi[1]})" if wpi else gap("no free source")],
    ["India GDP Growth", f"{gdp[0]:.1f}% ({gdp[1]})" if gdp else gap("no free source")],
    ["US CPI Index", f"{cpi:.1f}" if cpi is not None else gap("add FRED key")],
    ["US Unemployment %", f"{un:.1f}" if un is not None else gap("add FRED key")],
    ["India Forex Reserves", f"${fxr[0]/1e9:,.0f}B ({fxr[1]})" if fxr else gap("no free source")],
])
...
with st.sidebar:
    ...
    if HAS_XLSX:
        try:
            st.download_button(
                "Excel (.xlsx)", data=build_excel(_REPORT, _meta),
    ...
    if HAS_PDF:
        try:
            st.download_button(
                "PDF (.pdf)", data=build_pdf(_REPORT, _meta),
