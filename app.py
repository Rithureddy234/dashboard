...
render_table(["Country", "1Y", "2Y", "10Y"], [
    [_link("India", _RATE_SRC["India"]), ycell("india_1y"), ycell("india_2y"), _in10],
    [_link("US", _RATE_SRC["US"]), _us_cell("1Y", "DGS1"), _us_cell("2Y", "DGS2"), _us_cell("10Y", "DGS10")],
    [_link("UK", _RATE_SRC["UK"]), ycell("uk_1y"), ycell("uk_2y"), _uk10],
    [_link("Germany", _RATE_SRC["Germany"]), _de_cell("1Y"), _de_cell("2Y"), _de_cell("10Y")],
    [_link("Japan", _RATE_SRC["Japan"]), _jp_cell("1Y"), _jp_cell("2Y"), _jp10],
])
st.markdown('<div class="src">US = Treasury.gov (FRED fallback); Germany = ECB '
            'euro-area AAA curve; Japan = MOF JGB curve; India &amp; UK 10Y = FRED. '
            'India/UK 1Y/2Y come from a headless browser (work locally, blank on '
            'Streamlit Cloud — no free HTTP feed for those short-tenor yields).</div>',
            unsafe_allow_html=True)
