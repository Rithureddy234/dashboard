...
def bond_yield_yahoo(ticker):
    """Read a government-bond yield from Yahoo Finance (plain HTTP, works on
    Streamlit Cloud). Yahoo tickers for bond yields: IN1YT=RR, IN2YT=RR,
    GB1YT=RR, GB2YT=RR, etc. Returns float or None."""
    if not HAS_YF:
        return None
    try:
...
@st.cache_data(ttl=3600)  # 1h — yields move daily; hourly is plenty
def bond_yield_change_yahoo(ticker):
    """Like bond_yield_yahoo but also returns the day's change (last - prev).
    Returns (value, change) where change is in yield-points (e.g. -0.02 = -2bps).
    ...
    try:
        ...
        if len(h) >= 2:
            val = float(h.iloc[-1])
            prev = float(h.iloc[-2])
            val = val if 0 < val < 15 else None
            change = round(val - prev, 3) if val is not None else None
            # ignore implausibly large daily move (>1 percentage point)
            if change is not None and abs(change) >= 1.0:
                change = None
            return (val, change)
        elif len(h) == 1:
            val = float(h.iloc[-1])
            return (val if 0 < val < 15 else None, None)
    except Exception:
        pass
    return (None, None)
...
# return None immediately (Playwright not installed). Yahoo-based functions
# above are used instead for India/UK yields.
# ---------------------------------------------------------------------------
...
# Yahoo Finance government-bond yield tickers (plain HTTP, work on Streamlit Cloud).
# Format: <country><tenor>T=RR   e.g. IN1YT=RR = India 1-Year Treasury yield.
# These are the same tickers Bloomberg/Reuters distribute via Yahoo's free feed.
YIELD_TICKERS = {
    "india_1y": "IN1YT=RR",
    "india_2y": "IN2YT=RR",
    "uk_1y":    "GB1YT=RR",
...
_yld = {k: bond_yield_yahoo(tk) for k, tk in YIELD_TICKERS.items()}
_yld10 = {k: bond_yield_change_yahoo(tk) for k, tk in YIELD_10Y_TICKERS.items()}

# Keep YIELD_PAGES / YIELD_10Y_PAGES so the expander below still works.
YIELD_PAGES = {k: f"Yahoo: {tk}" for k, tk in YIELD_TICKERS.items()}
YIELD_10Y_PAGES = {k: f"Yahoo: {tk}" for k, tk in YIELD_10Y_TICKERS.items()}
...
st.markdown('<div class="src">US = Treasury.gov (FRED fallback); Germany = ECB '
            'euro-area AAA curve; Japan = MOF JGB curve; India &amp; UK all tenors = '
            'Yahoo Finance bond-yield feed (IN1YT=RR / GB1YT=RR style tickers, '
            'works on Streamlit Cloud).</div>',
            unsafe_allow_html=True)
...
with st.expander("Yield scrape details (verify the picked value)"):
    st.caption("India/UK/Japan 1Y, 2Y & 10Y come from Yahoo Finance bond-yield tickers "
               "(e.g. IN1YT=RR, GB1YT=RR). These work on Streamlit Cloud — no browser needed. "
               "A gap means Yahoo returned no data for that ticker today.")
